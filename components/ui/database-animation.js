...
class DatabaseAnimation {
  ...
  startCircleAnimations() {
    // Get all circles
    const circles = this.container.querySelectorAll('.database-circle');
    
    // Animate each circle with a slight delay
    circles.forEach((circle, index) => {
      // Set initial animation
      this.animateCircle(circle, index);
  ...
  animateCircle(circle, index) {
    ...
    setTimeout(() => {
      ...
      const interval = setInterval(() => {
        if (scaleIndex < scales[index].length) {
          circle.style.transform = `scale(${scales[index][scaleIndex]})`;
          scaleIndex++;
        } else {
